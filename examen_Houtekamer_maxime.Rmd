...
```{r t hell}
factorlist<-c((rep("d5h",24)),(rep("d5sh",18)),(rep("d3h",24)),(rep("d3sh",24)))
variablelist<-c(d5h[,3],d5sh[,3],d3h[,3],d3sh[,3])
d1<-data.frame(factorlist,variablelist)
colnames(d1)<-c("exp","values")
f1<-d1$values~d1$exp
```
...
print(ggplot(melted,aes(x=x,y=value,fill=variable)) + geom_bar(stat="identity", alpha=1),main="Activation of brain areas after training",
   xlab="", ylab="Level of activation")
```
